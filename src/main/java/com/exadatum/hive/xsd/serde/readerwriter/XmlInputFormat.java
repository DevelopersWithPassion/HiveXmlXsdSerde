...
public class XmlInputFormat extends TextInputFormat {
    ...
    public static class XmlRecordReader implements RecordReader<LongWritable, Text> {
        ...
        public XmlRecordReader(FileSplit input, JobConf jobConf) throws IOException {
            ...
            String configuredStartTag = conf.get(START_TAG_KEY);
            String configuredEndTag = conf.get(END_TAG_KEY);

            if (StringUtils.isEmpty(configuredEndTag) || StringUtils.isEmpty(configuredStartTag)) {
                this.startTag = XmlInputFormat.startTag.getBytes("utf-8");
                this.endTag = XmlInputFormat.endTag.getBytes("utf-8");
            } else {
